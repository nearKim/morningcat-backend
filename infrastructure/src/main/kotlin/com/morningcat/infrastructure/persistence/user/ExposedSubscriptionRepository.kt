...
class ExposedSubscriptionRepository(
    ...
    private fun ResultRow.toSubscription(): Subscription {
        val deliveryChannels =
            json
                .decodeFromString<List<String>>(this[Subscriptions.deliveryChannels])
                .map { channelName ->
                    when (channelName) {
        ...
        val contentPreferences =
            json
                .decodeFromString<Map<String, Boolean>>(this[Subscriptions.contentPreferences])
                .mapKeys { (key, _) ->
                    ContentCategory.valueOf(key)
                }

        val financialPreferences =
            json
                .decodeFromString<List<String>>(this[Subscriptions.financialPreferences])
                .toSet()
